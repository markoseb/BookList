...
class BookTest(BaseTest):
    def test_crud(self):
        book = Book("test", "testAuthor",
                    ...
                    "pl")
        self.assertIsNone(Book.find_by_isbn(9788326419935), f"Found an book with title {book.title}, but expected not to")

        book.save_to_db()

        self.assertIsNotNone(Book.find_by_isbn(9788326419935), f" Didn't Found an book with title {book.title}, but expected to")
